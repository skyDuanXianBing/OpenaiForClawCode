...
import { getRuntimeModelSettingFromSelectedModel } from '../../utils/model/runtimeSelectedModel.js'
...
function buildInheritedCliFlags(options?: {
  ...
  const modelOverride = getRuntimeModelOverride()
  if (modelOverride) {
...
async function handleSpawnSplitPane(
  ...
  const model = resolveTeammateModel(
    ...
    getRuntimeModelSettingFromSelectedModel(
      currentAppState.selectedModel,
      currentAppState.selectedModelForSession,
    ),
  )
