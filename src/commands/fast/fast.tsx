...
function applyFastMode(enable: boolean, setAppState: (f: (prev: AppState) => AppState) => void): void {
  ...
  if (enable) {
    setAppState(prev => {
      const currentModel = getRuntimeModelSettingFromSelectedModel(prev.selectedModel, prev.selectedModelForSession);
      // Only switch model if current model doesn't support fast mode
      const needsModelSwitch = !isFastModeSupportedByModel(currentModel);
      return {
        ...prev,
        ...(needsModelSwitch ? {
          selectedModel: createRuntimeSelectedModelFromValue(getFastModeModel(), prev.selectedModelForSession ?? prev.selectedModel),
...
function _temp(s) {
  return getRuntimeModelSettingFromSelectedModel(s.selectedModel, s.selectedModelForSession);
}
async function handleFastModeShortcut(enable: boolean, getAppState: () => AppState, setAppState: (f: (prev: AppState) => AppState) => void): Promise<string> {
  ...
  const mainLoopModel = getRuntimeModelSettingFromSelectedModel(currentState.selectedModel, currentState.selectedModelForSession);
